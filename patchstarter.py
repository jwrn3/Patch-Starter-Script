...
This script makes the following assumptions about the software:
    ...
    * Because the "publisher" cannot be reliably derived
      from Info.plist it is left blank unless passed as
      an argument
...
for releases of an app other than what is present on your system.""",
    ...
    parser.add_argument(
        "--app-version",
        help="Provide the version of the app (override " "CFBundleShortVersionString)",
        type=str,
        default="",
        metavar="<version>",
    )
    ...
    parser.add_argument(
        "--patch-only",
        help="Only create a patch, not a full definition",
        action="store_true",
    )
...
def make_definition(args):
    ...
    try:
        ...
        print(f"Could not read file: {info_plist_path}")
        raise SystemExit(1)
    ...
    if args.app_version:
        ...
        try:
            ...
            print(
                "Could not find 'CFBundleShortVersionString', please provide "
    ...
    if args.min_sys_version:
        ...
        try:
            ...
            print(
                "Could not find 'LSMinimumSystemVersion', please provide a "
                "value for the minimum OS version"
            )
            app_min_os = input("[10.9]: ") or "10.9"

    app_last_modified = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
    app_timestamp = datetime.utcfromtimestamp(os.path.getmtime(args.path)).strftime(
        "%Y-%m-%dT%H:%M:%SZ"
    )
    ...
    patch = {
        ...
        "reboot": False,
        "killApps": [{"bundleId": app_bundle_id, "appName": app_filename}],
        "components": [
            {
                "name": app_name,
                "version": app_version,
